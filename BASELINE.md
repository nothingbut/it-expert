...
- **开始日期**: 2026-03-27
- **预计完成**: 2026-03-27（5-20 分钟）
- **当前状态**: 🔄 待开始
- **实际情况**: RDP 微软账户登录失败，需要替代方案
...
联想 ThinkBook+ (客户端) → 铭凡 UM773 (服务端)
    ...
    |       +--- 部署：5 分钟
    |
    +--- 备选方案 1：Windows RDP (如账户问题解决)
    |       +--- 限制：微软账户登录失败 ❌
    |       +--- 只能用本地账户
    |
    +--- 备选方案 2：Moonlight + Sunshine (游戏性能最佳)
            +--- 适合：经常游戏，需要极致性能
            +--- 部署：10 分钟
```

#### 远程桌面方案对比（针对你的情况）
| 方案 | 访问当前会话 | 微软账户支持 | 游戏性能 | 远程访问 | 费用 | 推荐度 |
|------|------------|-------------|---------|---------|------|--------|
| **RustDesk** ⭐ | ✅ 是 | ✅ 无需登录 | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | 免费 | ⭐⭐⭐⭐⭐ |
| Windows RDP | ❌ 新会话 | ❌ 登录失败 | ⭐⭐ | ⭐⭐⭐⭐ | 免费 | ⭐⭐ |
| Moonlight + Sunshine | ❌ 新会话 | ❌ 需配置 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | 免费 | ⭐⭐⭐⭐ |

**说明**：
- RustDesk 直接连接当前桌面会话，无需重新登录
- 可以访问微软账户登录后的所有应用和配置
- 完美解决 RDP 账户登录问题

#### 阶段进度

##### 阶段 0: RustDesk 快速部署（P0 - 推荐首选）⭐
| 任务 | 状态 | 完成日期 | 备注 |
|------|------|----------|------|
| 0.1 下载安装 RustDesk（铭凡） | ⬜ 待开始 | - | https://rustdesk.com |
| 0.2 记录桌面 ID 和设置密码 | ⬜ 待开始 | - | 永久密码 |
| 0.3 配置无人值守访问 | ⬜ 待开始 | - | 权限设置 |
| 0.4 下载安装 RustDesk（ThinkBook+） | ⬜ 待开始 | - | 同版本 |
| 0.5 添加到地址簿 | ⬜ 待开始 | - | 保存密码 |
| 0.6 连接测试 | ⬜ 待开始 | - | 验证当前会话 |
| 0.7 性能优化 | ⬜ 待开始 | - | 直接连接、质量优先 |
| **阶段 0 完成度** | **0/7 (0%)** | - | 解决 RDP 账户问题 |

##### 阶段 1: Windows RDP 配置（P2 - 备选，如账户问题解决）
| 任务 | 状态 | 完成日期 | 备注 |
|------|------|----------|------|
| 1.1 解决微软账户登录问题 | ⬜ 待开始 | - | 见下方故障排除 |
| 1.2 启用远程桌面（铭凡） | ⬜ 待开始 | - | 设置 → 系统 → 远程桌面 |
| 1.3 配置 NLA 身份验证 | ⬜ 待开始 | - | 安全性增强 |
| 1.4 获取铭凡 IP 地址 | ⬜ 待开始 | - | ipconfig |
| 1.5 测试 RDP 服务 | ⬜ 待开始 | - | Test-NetConnection |
| 1.6 客户端连接测试（ThinkBook+） | ⬜ 待开始 | - | mstsc |
| 1.7 安装 Microsoft Remote Desktop | ⬜ 待开始 | - | Microsoft Store |
| 1.8 创建连接配置 | ⬜ 待开始 | - | 保存凭据 |
| 1.9 创建桌面快捷方式 | ⬜ 待开始 | - | .rdp 文件 |
| **阶段 1 完成度** | **0/9 (0%)** | - | 暂时跳过 |

##### 阶段 2A: Moonlight + Sunshine 游戏串流（P1 - 免费推荐）⭐
| 任务 | 状态 | 完成日期 | 备注 |
|------|------|----------|------|
| 2A.1 下载安装 Sunshine（铭凡） | ⬜ 待开始 | - | GitHub Releases |
| 2A.2 配置 Sunshine Web 界面 | ⬜ 待开始 | - | https://localhost:47990 |
| 2A.3 配置视频设置 | ⬜ 待开始 | - | H.265, 1080p, 60fps |
| 2A.4 添加应用 | ⬜ 待开始 | - | Desktop/Playnite/Steam |
| 2A.5 配置防火墙 | ⬜ 待开始 | - | 端口 47989-47990 |
| 2A.6 安装 Moonlight（ThinkBook+） | ⬜ 待开始 | - | moonlight-stream.org |
| 2A.7 配对服务器 | ⬜ 待开始 | - | 输入 PIN 码 |
| 2A.8 游戏性能测试 | ⬜ 待开始 | - | 延迟 < 5ms |
| **阶段 2A 完成度** | **0/8 (0%)** | - | 完全免费无限制 |

##### 阶段 2B: Parsec 游戏串流（P1 - 备选）
| 任务 | 状态 | 完成日期 | 备注 |
|------|------|----------|------|
| 2B.1 下载安装 Parsec（铭凡） | ⬜ 待开始 | - | https://parsec.app |
| 2B.2 注册并登录账号 | ⬜ 待开始 | - | 免费账号 |
| 2B.3 配置 Hosting 设置 | ⬜ 待开始 | - | 1080p 60fps |
| 2B.4 验证 Hosting 状态 | ⬜ 待开始 | - | Ready to Host |
| 2B.5 安装客户端（ThinkBook+） | ⬜ 待开始 | - | 同账号登录 |
| 2B.6 局域网连接测试 | ⬜ 待开始 | - | 自动发现 |
| 2B.7 配置快捷键 | ⬜ 待开始 | - | Ctrl+Alt+Shift+D |
| 2B.8 游戏性能测试 | ⬜ 待开始 | - | 延迟 < 10ms |
| **阶段 2B 完成度** | **0/8 (0%)** | - | 免费版有限制 |
...
#### 总体进度
- **总任务数**: 38（RustDesk 7 + RDP 9 + 游戏方案 16 + VPN 7）
- **已完成**: 0
- **进行中**: 0
- **未开始**: 38
- **总完成度**: **0%**
...
**方案 B：高性能游戏方案** ⏱️ 15 分钟
- **阶段 0（RustDesk）**: 日常操作
- **阶段 2A（Moonlight）**: 游戏场景
  - 性能极致（< 5ms）
  - 完全免费无限制
...
- **阶段 0（RustDesk）**: 首选推荐 ⭐⭐⭐
- **阶段 1（RDP）**: 跳过（账户问题）
- **阶段 2A（Moonlight）**: 游戏性能最佳（可选）
- **阶段 2B（Parsec）**: 备选方案
- **阶段 3（Tailscale）**: RustDesk 已自带穿透

#### 验收标准
| 测试项 | 目标 | 状态 |
|--------|------|------|
| **RustDesk 连接** | < 5 秒连接 | ⬜ 待测试 |
| **RustDesk 访问当前会话** | ✅ 微软账户桌面 | ⬜ 待测试 |
| **RustDesk 应用可用** | ✅ Playnite 等应用 | ⬜ 待测试 |
| **RustDesk 延迟** | < 20ms | ⬜ 待测试 |
| **RustDesk 游戏** | 30-60fps | ⬜ 待测试 |
| RustDesk 远程访问 | 自动穿透 | ⬜ 待测试 |
| RustDesk 剪贴板 | 双向复制 | ⬜ 待测试 |
| RDP 微软账户登录 | 解决问题 | ⬜ 待修复 |
| Moonlight 游戏延迟 | < 5ms | ⬜ 待测试 |
| Moonlight 帧率 | 60fps | ⬜ 待测试 |
...
| Sunshine (Moonlight) | - | 47989-47990 | TCP | ⬜ 未配置 |
| Sunshine (Moonlight) | - | 47998-48010 | UDP | ⬜ 未配置 |
| Parsec（备选） | - | UDP 8000-8010 | UDP | ⬜ 未配置 |
...
| Sunshine TCP | 47989-47990 | Inbound | Allow | ⬜ 未配置 |
| Sunshine UDP | 47998-48010 | Inbound | Allow | ⬜ 未配置 |
| Parsec（备选） | 8000-8010 | Inbound | Allow | ⬜ 未配置 |
...
- **[创建]** 远程桌面部署计划（remote-desktop-deployment-plan.md）
  ...
  - 日常使用指南和故障排除
- **[新增]** 完全免费游戏串流方案 ⭐⭐⭐
  - 推荐：Moonlight + Sunshine（完全免费开源）
  - 备选：Parsec 免费版（有会话时长限制）
  - 备选：Rustdesk（完全免费，性能略逊）
  - 性能：延迟 < 5ms，支持 4K 120fps
  - 优势：零成本、无限制、无广告
- **[创建]** 免费游戏串流方案文档（docs/free-gaming-remote-solutions.md）
  - Moonlight + Sunshine 详细部署步骤
  - Rustdesk 快速部署指南
  - 3 种方案性能对比（延迟、帧率、带宽）
  - 使用场景推荐和成本分析（完全免费）
  - 故障排除和优化建议
- **[创建]** 免费游戏串流快速开始（QUICKSTART-FREE-GAMING.md）
  - 10 分钟快速部署 Moonlight + Sunshine
  - 2 步配置（服务端 5-8 分钟 + 客户端 3-5 分钟）
  - 常见问题快速修复
  - 与 Parsec 对比说明
- **[更新]** 远程桌面项目方案架构
  - 添加游戏方案二选一：Moonlight + Sunshine（推荐）vs Parsec
  - 更新总任务数：23 → 31（增加 Moonlight 部署阶段）
  - 添加 Sunshine 端口配置（TCP 47989-47990, UDP 47998-48010）
  - 更新验收标准（添加 Moonlight 性能测试）
- **[调整]** 远程桌面方案架构（针对 RDP 账户问题）⭐⭐⭐
  - **问题**：RDP 微软账户登录失败，只能用本地账户
  - **影响**：无法访问微软账户的应用和配置
  - **解决方案**：RustDesk 作为首选方案
  - **优势**：直接连接当前会话，访问所有应用配置
  - 更新总任务数：31 → 38（增加 RustDesk 阶段 0）
- **[新增]** RustDesk 作为主要远程桌面方案 ⭐⭐⭐
  - 推荐度：⭐⭐⭐⭐⭐（首选，完美解决 RDP 问题）
  - 部署时间：5 分钟
  - 核心优势：
    - 直接连接当前桌面会话（解决 RDP 新会话问题）
    - 无需重新登录，访问微软账户所有应用
    - 完全免费无限制（无任何会话时长限制）
    - 自动 P2P 穿透（无需端口转发或 VPN）
    - 一站式解决：日常操作 + 游戏 + 远程访问
  - 性能：延迟 10-20ms（局域网），30-60fps 游戏
- **[创建]** RustDesk 快速部署指南（QUICKSTART-RUSTDESK.md）
  - 5 分钟快速部署流程
  - 2 步配置（服务端 2 分钟 + 客户端 2 分钟）
  - 针对 RDP 账户问题的解决方案说明
  - 性能优化和故障排除指南
  - 与 RDP/Moonlight 对比分析
  - 可选：自建中转服务器（隐私增强）
- **[创建]** RustDesk 自建服务器部署指南（rustdesk-server-deployment.md）⭐
  - 在飞牛NAS上部署 RustDesk 中转服务器
  - Docker 一键部署（15-20 分钟）
  - 完整网络配置（防火墙、端口转发）
  - 客户端配置指南（局域网 + 外网）
  - 优势：
    - 完全自主可控
    ...
    - 不依赖公共服务器
    - 可能更快（局域网内直连）
  - 服务器组件：hbbs（ID/信令）+ hbbr（中继）
  - 端口：21115-21119（5个端口）
- **[更新]** 远程桌面项目优先级调整
  - **P0**：RustDesk（5 分钟）- 首选推荐 ⭐⭐⭐
  - **P1**：Moonlight + Sunshine（10 分钟）- 游戏性能最佳（可选）
  - **P2**：Windows RDP（跳过）- 账户问题未解决
  - **P2**：Tailscale VPN（跳过）- RustDesk 已自带穿透
- **[配置]** 更新网络配置参数
  - 添加 RDP 端口 3389
  - 添加 Parsec 端口 UDP 8000-8010
  - 添加 Sunshine 端口（TCP 47989-47990, UDP 47998-48010）
  - 添加防火墙规则配置
...
#### 远程桌面项目
- **[RustDesk 快速开始 (QUICKSTART)](./QUICKSTART-RUSTDESK.md)** ⭐⭐⭐ 首选推荐 - 5 分钟部署，解决 RDP 账户问题 🚀
- **[RustDesk 自建服务器部署](./rustdesk-server-deployment.md)** 🔒 隐私增强 - 在飞牛NAS上部署私有服务器
- **[免费游戏串流快速开始 (QUICKSTART)](./QUICKSTART-FREE-GAMING.md)** - 10 分钟部署 Moonlight + Sunshine 🎮
- **[RDP 快速开始指南](./QUICKSTART-REMOTE-DESKTOP.md)** - 5 分钟快速配置（如 RDP 账户问题解决）
- **[免费游戏串流方案对比](./docs/free-gaming-remote-solutions.md)** - 完全免费方案详细对比 ⭐
- **[远程桌面方案对比](./docs/remote-desktop-solutions.md)** - 6 种方案详细对比和选型 🖥️
- **[远程桌面部署计划](./remote-desktop-deployment-plan.md)** - 完整部署流程参考
